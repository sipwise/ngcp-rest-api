...
export class ResellersMockRepository implements ResellersRepository {
    private readonly resellerDB: ResellersMockDB
    ...
    contractExists(contractId: number): Promise<boolean> {
        return Promise.resolve(this.contractsDB[contractId] != undefined)
    }

    contractHasSystemContact(contractId: number): Promise<boolean> {
        return Promise.resolve(this.contractsDB[contractId].contact_id != undefined)
    }
