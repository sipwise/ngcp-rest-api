...
export class ContractsMockRepository implements ContractsRepository {
    ...
    create(entity: internal.Contract, sr: ServiceRequest): Promise<internal.Contract> {
        const nextId = this.getNextId(this.contractsDB)
        entity.id = nextId
        this.contractsDB[nextId] = entity

        return Promise.resolve(entity)
    }

    delete(id: number, sr: ServiceRequest): Promise<number> {
        this.throwErrorIfIdNotExists(this.contractsDB, id)
        return Promise.resolve(1)
    }

    read(id: number, sr: ServiceRequest): Promise<internal.Contract> {
        this.throwErrorIfIdNotExists(this.contractsDB, id)
        return Promise.resolve(this.contractsDB[id])
    }
    ...
    readAll(sr: ServiceRequest): Promise<[internal.Contract[], number]> {
        const contracts: [internal.Contract[], number] =
            [Object.keys(this.contractsDB).map(id => this.contractsDB[id]), Object.keys(this.contractsDB).length]
        return Promise.resolve(contracts)
    }

    readProductByType(type: string, sr: ServiceRequest): Promise<internal.Product> {
        for (const key of Object.keys(this.productsDB)) {
            const id: number = +key
            const product: internal.Product = this.productsDB[id]
